...
function get(p) {
    ...
    for (i = 0; i < tovars.length; i++) {
        ...
        str += "<div class=\"item__stretchy-wrapper\"><img src=\"" + tovars[i].getElementsByTagName("urlimg")[0].textContent + "\"></div><h4 class=\"item__title\">" + tovars[i].getElementsByTagName("name")[0].textContent + "</h4><p class=\"item__cost\">" + cost +  " " + tovars[i].getElementsByTagName("cost")[0].getAttribute("unit") + "</p><p class=\"item__scidka\">" + cost_s.toFixed(2) + " " + tovars[i].getElementsByTagName("cost")[0].getAttribute("unit") + "</p><a href=\"tovar.html?category="+ p + "&tovar=" + i + "\"class=\"item__button\">Купить</a>";
        str += "</div></div>";
    }
